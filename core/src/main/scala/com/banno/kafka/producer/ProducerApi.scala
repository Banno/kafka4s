...
trait ProducerApi[F[_], K, V] {
  ...
  private[producer] def sendRaw(record: ProducerRecord[K, V], callback: Callback): JFuture[RecordMetadata]
...
object ProducerApi {
  ...
  def createProducer[F[_]: Sync, K, V](keySerializer: Serializer[K], valueSerializer: Serializer[V], configs: (String, AnyRef)*): F[KafkaProducer[K, V]] = 
  ...
  def apply[F[_]: Async, K, V](keySerializer: Serializer[K], valueSerializer: Serializer[V], configs: (String, AnyRef)*): F[ProducerApi[F, K, V]] = 
    createProducer[F, K, V](keySerializer, valueSerializer, configs: _*).map(ProducerImpl[F, K, V](_))

  def create[F[_]: Async, K: Serializer, V: Serializer](configs: (String, AnyRef)*): F[ProducerApi[F, K, V]] = 
    apply[F, K, V](implicitly[Serializer[K]], implicitly[Serializer[V]], configs: _*)

  def shifting[F[_]: Async: ContextShift, K, V](producerContext: ExecutionContext, configs: (String, AnyRef)*): F[ProducerApi[F, K, V]] =
    apply[F, K, V](configs: _*).map(ShiftingProducerImpl[F, K, V](_, producerContext))
  def shifting[F[_]: Async: ContextShift, K, V](producerContext: ExecutionContext, keySerializer: Serializer[K], valueSerializer: Serializer[V], configs: (String, AnyRef)*): F[ProducerApi[F, K, V]] =
    apply[F, K, V](keySerializer, valueSerializer, configs: _*).map(ShiftingProducerImpl[F, K, V](_, producerContext))

  def createShifting[F[_]: Async: ContextShift, K: Serializer, V: Serializer](producerContext: ExecutionContext, configs: (String, AnyRef)*): F[ProducerApi[F, K, V]] =
  ...
  def createGenericProducer[F[_]: Sync](configs: (String, AnyRef)*): F[KafkaProducer[GenericRecord, GenericRecord]] = 
  ...
  def generic[F[_]: Async](configs: (String, AnyRef)*): F[ProducerApi[F, GenericRecord, GenericRecord]] = 
    createGenericProducer[F](configs: _*).map(ProducerImpl[F, GenericRecord, GenericRecord](_))

  def avro4s[F[_]: Async, K: ToRecord, V: ToRecord](configs: (String, AnyRef)*): F[ProducerApi[F, K, V]] = 
    generic[F](configs: _*).map(Avro4sProducerImpl[F, K, V](_))

  def avro4sShifting[F[_]: Async: ContextShift, K: ToRecord, V: ToRecord](producerContext: ExecutionContext, configs: (String, AnyRef)*): F[ProducerApi[F, K, V]] =
