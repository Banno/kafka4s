...
import org.apache.kafka.clients.consumer.{ConsumerRecords, ConsumerRecord, OffsetAndMetadata}
import com.sksamuel.avro4s.{ToRecord, FromRecord}
import cats._
import scala.collection.JavaConverters._
import java.lang.{Double => JDouble, Float => JFloat, Integer => JInteger, Long => JLong, Short => JShort}
...
package object kafka {
  ...
  implicit class ScalaProducerRecord[K, V](pr: ProducerRecord[K, V]) {
    ...
    def toGenericRecord(implicit ktr: ToRecord[K], vtr: ToRecord[V]): ProducerRecord[GenericRecord, GenericRecord] = 
  ...
  implicit class ScalaConsumerRecord[K, V](cr: ConsumerRecord[K, V]) {
    ...
    def offsetMap: Map[TopicPartition, OffsetAndMetadata] = Map(new TopicPartition(cr.topic, cr.partition) -> new OffsetAndMetadata(cr.offset))
    def nextOffset: Map[TopicPartition, OffsetAndMetadata] = Map(new TopicPartition(cr.topic, cr.partition) -> new OffsetAndMetadata(cr.offset + 1))
    ...
    def bimap[K2, V2](f: K => K2, g: V => V2): ConsumerRecord[K2, V2] = 
      new ConsumerRecord(cr.topic, cr.partition, cr.offset, cr.timestamp, cr.timestampType, /*cr.checksum*/-1, cr.serializedKeySize, cr.serializedValueSize, f(cr.key), g(cr.value), cr.headers)
  }
  ...
  implicit class GenericConsumerRecord(cr: ConsumerRecord[GenericRecord, GenericRecord]) {
    ...
    /** This only works when both key and value are non-null. */
    def fromGenericRecord[K, V](implicit kfr: FromRecord[K], vfr: FromRecord[V]): ConsumerRecord[K, V] = 
  ...
  implicit class GenericConsumerRecords(crs: ConsumerRecords[GenericRecord, GenericRecord]) {
    def fromGenericRecords[K: FromRecord, V: FromRecord]: ConsumerRecords[K, V] = 
      new ConsumerRecords[K, V](crs.partitions.asScala.map(tp => (tp, crs.records(tp).asScala.map(_.fromGenericRecord[K, V]).asJava)).toMap.asJava)
  }

  implicit def eqProducerRecord[K, V]: Eq[ProducerRecord[K, V]] = Eq.fromUniversalEquals // ProducerRecord implements equals properly
  
  implicit def eqConsumerRecord[K, V]: Eq[ConsumerRecord[K, V]] = new Eq[ConsumerRecord[K, V]] { // ConsumerRecord does not implement equals :(
  ...
  implicit def stringSerializer: Serializer[String] = new StringSerializer() //this will always use UTF8 encoding
  ...
  implicit def doubleSerializer: Serializer[Double] = Contravariant[Serializer].contramap(jdoubleSerializer)(Double.box)
  implicit def floatSerializer: Serializer[Float] = Contravariant[Serializer].contramap(jfloatSerializer)(Float.box)
  implicit def intSerializer: Serializer[Int] = Contravariant[Serializer].contramap(jintegerSerializer)(Int.box)
  implicit def longSerializer: Serializer[Long] = Contravariant[Serializer].contramap(jlongSerializer)(Long.box)
  implicit def shortSerializer: Serializer[Short] = Contravariant[Serializer].contramap(jshortSerializer)(Short.box)

  implicit def doubleDeserializer: Deserializer[Double] = Functor[Deserializer].map(jdoubleDeserializer)(Double.unbox)
  implicit def floatDeserializer: Deserializer[Float] = Functor[Deserializer].map(jfloatDeserializer)(Float.unbox)
  implicit def intDeserializer: Deserializer[Int] = Functor[Deserializer].map(jintegerDeserializer)(Int.unbox)
  implicit def longDeserializer: Deserializer[Long] = Functor[Deserializer].map(jlongDeserializer)(Long.unbox)
  implicit def shortDeserializer: Deserializer[Short] = Functor[Deserializer].map(jshortDeserializer)(Short.unbox)
}
