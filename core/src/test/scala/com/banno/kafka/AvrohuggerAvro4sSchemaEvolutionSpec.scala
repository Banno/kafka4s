...
import io.confluent.kafka.serializers.{KafkaAvroSerializer, KafkaAvroDeserializer, AbstractKafkaAvroSerDeConfig}
import io.confluent.kafka.schemaregistry.client.MockSchemaRegistryClient
import org.apache.avro._
import org.apache.avro.generic.GenericRecord
import com.sksamuel.avro4s.{SchemaFor, RecordFormat}
import com.banno._
...
object Compatibility {
  ...
  def compatible(validator: SchemaValidator, newSchema: Schema, previousSchemas: Seq[Schema]): Boolean = 
    try {
  ...
  def compatible(validator: SchemaValidator, newSchema: Schema, previousSchema: Schema): Boolean = compatible(validator, newSchema, Seq(previousSchema))
  def backwardCompatible(newSchema: Schema, previousSchema: Schema): Boolean = compatible(backward, newSchema, previousSchema)
  def backwardTransitiveCompatible(newSchema: Schema, previousSchema: Schema): Boolean = compatible(backwardTransitive, newSchema, previousSchema)
  def forwardCompatible(newSchema: Schema, previousSchema: Schema): Boolean = compatible(forward, newSchema, previousSchema)
  def forwardTransitiveCompatible(newSchema: Schema, previousSchema: Schema): Boolean = compatible(forwardTransitive, newSchema, previousSchema)
  def fullCompatible(newSchema: Schema, previousSchema: Schema): Boolean = compatible(full, newSchema, previousSchema)
  def fullTransitiveCompatible(newSchema: Schema, previousSchema: Schema): Boolean = compatible(fullTransitive, newSchema, previousSchema)
}

class AvrohuggerAvro4sSchemaEvolutionSpec extends PropSpec with ScalaCheckDrivenPropertyChecks with Matchers {

  val client = new MockSchemaRegistryClient()
  val configs = Map(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG -> "http://inmemorytest").asJava
  ...
  property("Adding a field without a default is forward, but not backward, compatible") {
    ...
    val b1 = serializer.serialize(topic, rf1.to(r1))
    rf1.from(deserializer.deserialize(topic, b1).asInstanceOf[GenericRecord]) should === (r1)
    a [NullPointerException] should be thrownBy rf2.from(deserializer.deserialize(topic, b1).asInstanceOf[GenericRecord])
  ...
  property("Removing a field without a default is backward, but not forward, compatible") {
    ...
    val b2 = serializer.serialize(topic, rf2.to(r2))
    a [NullPointerException] should be thrownBy rf1.from(deserializer.deserialize(topic, b2).asInstanceOf[GenericRecord])
    rf2.from(deserializer.deserialize(topic, b2).asInstanceOf[GenericRecord]) should === (r2)
  }
  ...
  property("Adding a union type is backward, but not forward compatible") {
    ...
    a [RuntimeException] should be thrownBy rf1.from(deserializer.deserialize(topic, b3).asInstanceOf[GenericRecord])
  ...
  property("Removing a union type is forward, but not backward compatible") {
    ...
    a [RuntimeException] should be thrownBy rf2.from(deserializer.deserialize(topic, b3).asInstanceOf[GenericRecord])
  }
