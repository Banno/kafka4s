...
case class ThrowOnSendProducer[K, V]() extends Producer[K, V] {
  ...
  def registerMetricForSubscription(x$1: org.apache.kafka.common.metrics.KafkaMetric): Unit = ???
  def unregisterMetricFromSubscription(x$1: org.apache.kafka.common.metrics.KafkaMetric): Unit = ???
}
